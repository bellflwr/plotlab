...
void PlotRender::draw_line(sf::RenderWindow& window, int ax, int ay, int bx,
                           int by, sf::Color color) {
...
void PlotRender::draw_arrow(sf::RenderWindow& window, int ax, int ay, int bx,
                            int by, sf::Color color) {
    ...
    draw_line(window, bx, by, bx - (int)std::round(side1.x),
              by - (int)std::round(side1.y));

    sf::Vector2f side2 = vec.normalized().rotatedBy(sf::degrees(-45)) * 10.F;
    draw_line(window, bx, by, bx - (int)std::round(side2.x),
              by - (int)std::round(side2.y));
}
